...
|       Na podstawie sumarycznych przychodów ze sprzedaży każdej gry, zwizualizowaliśmy ich zmianę w czasie. Rozważany horyzont czasowy wynosi 3 miesiące.
...
Analogicznie postępujemy w przypadku wypożyczeń.
...
JOIN rental
USING (item_id)
WHERE MONTH(rental_date) >= 3 AND YEAR(rental_date) = 2023"""
...
|       Aby odpowiedzieć na to pytanie, zliczamy ile gier z danej kategorii sprzedało się od początku historii sklepu. Na tej podstawie tworzymy ranking kategorii, do której gry należące są najczęściej kupowane.
...
|       W celu zobrazowania zmiany sprzedaży na przestrzeni lat prezentujemy wyniki w postaci wykresów. Przedstawiają one odpowiednio zmianę dziennego, oraz miesięcznego przychodu ze sprzedaży gier.
